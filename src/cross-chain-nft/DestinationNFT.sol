...
contract DestinationNFT is ERC721A, ERC721AQueryable, Ownable {
    ...
    function safeBtachTransfer(address to, uint256[] memory tokenIds) public payable returns (bytes32 messageID) {
        for (uint i; i < tokenIds.length; i++) {
            /// @dev Calling original 'transferFrom()' to avoid double cross-chain request
            /// @dev We cannot call 'safeTransferFrom(from,to,tokenId)' or 'safeTransferFrom(from,to,tokenId,_data)' here as those are overridden by 'transferFrom()'
    ...
    function onGmpReceived(bytes32 id, uint128 network, bytes32 sender, bytes calldata data) external payable returns (bytes32) {
        ...
        /// @dev Minting all tokens exactly as they exist on source chain. We avoid need of additional mapping, but we bear additional cost of not using batchMint
    ...
    function _afterTokenTransfers(address from, address to, uint256 startTokenId, uint256 quantity) internal override(ERC721A) {
